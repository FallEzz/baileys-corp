...
/* =========================================================
 * Bagian di bawah ini digabung dari builder.js (NIXCODE)
 * Watermark & lisensi aslinya WAJIB tetap dipertahankan.
 * ========================================================= */
...
class Toolkit {
	...
	static async toUrl(_client, path, mediaType = 'document') {
		...
		const media = await Utils_1.prepareWAMessageMedia(
			{
...
class Button extends BaseBuilder {
	...
	async toCard() {
		return {
			...
			header: {
				...
				...(this._data
					? await Utils_1.prepareWAMessageMedia(this._data, { upload: this.#client.waUploadToServer }).catch((e) => {
	...
	async build(jid, { messageId, ...options } = {}) {
		...
		return Utils_1.generateWAMessageFromContent(
			jid,
			{
				...this._extraPayload,
				interactiveMessage: {
			...
			{ messageId: messageId || Utils_1.generateMessageIDV2(), ...options }
		);
	}
	...
	async build(jid, { messageId, ...options } = {}) {
		const _thumbnail = this._image?.is_raw
			...
			: this._image
				...
				: null;
		const msg = Utils_1.generateWAMessageFromContent(
			jid,
			{
				...this._extraPayload,
				buttonsMessage: {
...
class AIRich extends BaseBuilder {
	...
	async sendEdit(jid, id, { msg, messageId, additionalNodes = [], ...options } = {}) {
		...
		const msgEdit = await this.buildEdit(jid, id, {
			msg,
			messageId: messageId || Utils_1.generateMessageIDV2(),
			...options,
		});
...
const makeMessagesSocket = (config) => {
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            // --- Dukungan builder.js (Button, ButtonV2, Carousel, AIRich) ---
            // content di sini adalah instance builder, contoh: new Button(sock).setBody('...').addButton(...)
            const isBuilderInstance = content instanceof Button
                || content instanceof ButtonV2
                || content instanceof Carousel
                || content instanceof AIRich;

            if (isBuilderInstance) {
                // builder punya .send() sendiri yang sudah handle build() + relayMessage(),
                // karena instance-nya dibuat dengan client/sock yang sama (new Button(sock)).
                const builtMsg = await content.send(jid, {
                    messageId: options.messageId || Utils_1.generateMessageID(),
                ...
                if (config.emitOwnEvents) {
                    process.nextTick(() => {
                        processingMutex.mutex(() => upsertMessage(builtMsg, 'append'));
                    });
                }

                return builtMsg;
            }
            // --- akhir dukungan builder.js ---
...
exports.BUILDER_VERSION = VERSION;
